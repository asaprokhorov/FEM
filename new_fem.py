...
def _calculate_matrix_row_elements(i, nodes, basis, m, beta, sigma, alpha):
    ...
    if phi_i_left:
        ci_left = integrate.quad(
            lambda x: m(x) * phi_i_left(x, True) * phi_i(x, True) + beta(x) * phi_i_left(x, True) * phi_i(x) + sigma(x) * phi_i_left(
                x) * phi_i(x),
            xi_left, xi)[0]
    ci = integrate.quad(
        lambda x: m(x) * phi_i(x, True) ** 2 + beta(x) * phi_i(x, True) * phi_i(x) + sigma(x) * phi_i(x) ** 2, xi_left, xi_right
    )[0] + alpha * phi_i(nodes[-1]) ** 2
    ci_right = None
    if phi_i_right:
        ci_right = integrate.quad(
            lambda x: m(x) * phi_i(x, True) * phi_i_right(x, True) + beta(x) * phi_i_right(x, True) * phi_i(x) + sigma(x) * phi_i(
                x) * phi_i_right(x),
            xi, xi_right
        )[0]
...
def _create_matrix(nodes, basis, m, beta, sigma, f, alpha, _u):
    size = len(nodes)
    matrix = zeros((size, size))
    b = zeros(size)
    for i in range(size):
        ci_left, ci, ci_right = _calculate_matrix_row_elements(i, nodes, basis, m, beta, sigma, alpha)
        if ci_left:
...
def solve_fem(m, beta, sigma, f, alpha, _u, nodes):
    ...
    matrix, b = _create_matrix(nodes=nodes, basis=basis, m=m, beta=beta, sigma=sigma, f=f, alpha=alpha,
                               _u=_u)
    solution = linalg.solve(matrix, b)
    return lambda x: sum([basis[i](x) * solution[i] for i in range(len(solution))])


def solve_error(m, beta, sigma, f, alpha, _u, nodes, solution):
    basis = create_bubble_basis(nodes)
    size = len(nodes) - 1
    c = []
    for i in range(size):
        matrix_element = \
        integrate.quad(lambda x: m(x) * basis[i](x, True) ** 2 + beta(x) * basis[i](x, True) * basis[i](x) + sigma(x) * basis[i](x) ** 2, nodes[i], nodes[i + 1])[
            0] + \
        alpha * basis[i](nodes[-1]) ** 2

        vector_element = integrate.quad(
            lambda x: f(x) * basis[i](x) - m(x) * derivative(solution, x, dx=1e-6) * basis[i](x, True) - beta(x) * derivative(solution, x, dx=1e-6) * basis[i](x) - sigma(
